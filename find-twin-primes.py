...
# figure out whether an integer is a prime number
def isPrime(n):                          
    ...
    return True
# figure out whether two integers are twin primes
def isTwinPrime(n):
    if isPrime(n) and isPrime(n+2): return True   # if n and n+2 are both prime numbers, they are twin prime
# find out the total number and the maxmimum twin primes between 1k and 1m
j=0
import numpy as np
x=np.zeros(1000000)
...
print('number of twin primes=',j,'maxmimum twin primes=', x[j-1], x[j-1]+2,sep='\n' )

# result and output
# number of twin primes=
# 8134
# maxmimum twin primes=
# 999959.0
# 999961.0
